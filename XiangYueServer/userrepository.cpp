...
std::optional<UserRecord> UserRepository::findByUsername(const QString &username)
{
    ...
    q.prepare("SELECT id, username, password, avatar FROM users WHERE username=?");
...
{
    QSqlQuery q(DBManager::instance().db());
    q.prepare("SELECT id, username, password, avatar FROM users WHERE id=?");
    q.addBindValue(id);
...
{
    QSqlQuery q(DBManager::instance().db());
    q.prepare("INSERT INTO users(username, password, avatar) VALUES(?,?,?)");
    q.addBindValue(username);
