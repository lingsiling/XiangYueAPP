- # 享阅（XiangYueAPP / XiangYueServer）
  ...
  **关键特性**：支持 **10000+ 并发连接**、**多文件上传**、**资源评论系统**、**资源收藏系统**、**实时传输进度条**
  ...
  **关键保证**：只有收到 `UPLOAD_OK` 才发送下一个，保证状态一致。
  ...
  | **FileClient**           | 网络逻辑（LIST/UPLOAD/DOWNLOAD/评论/收藏协议） |
  | **ResourceDetailDialog** | 资源详情页（下载、评论显示、发送、收藏）       |
